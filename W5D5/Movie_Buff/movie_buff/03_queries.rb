...
def costars(name)
  ...
  # appeared with.
  # Hint: use a subquery
  actor_films = Movie.joins(:actors).where(actors: {name: name})
  Actor
    .joins(:movies)
    .where(movies: {id: actor_films})
    .where.not(name: name)
    .distinct
    .pluck(:name)
end
...
def starring(whazzername)
  ...
  match_string = "%" + whazzername.split('').join('%') + "%"
  Movie
    .joins(:actors)
    .where("actors.name ILIKE ?", match_string)
end
